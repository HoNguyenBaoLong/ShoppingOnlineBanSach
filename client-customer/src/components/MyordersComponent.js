    ...
    const orders = this.state.orders.map((item) => {
      return (
        <tr key={item._id} className="datatable" onClick={() => this.trItemClick(item)}>
          ...
          <td className='border-2 border-emerald-900 bg-gray-100 pl-2 pr-2 font-extrabold text-emerald-800'>{item.total} đ</td>
      ...
      var items = this.state.order.items.map((item, index) => {
        return (
          <tr key={item.product._id} className="datatable">
            ...
            <td className='border-2 border-sky-900 bg-sky-100 pl-2 pr-2 font-extrabold'>{item.product.price} đ</td>
            <td className='border-2 border-sky-900 bg-white pl-2 pr-2 font-medium'>{item.quantity} </td>
            <td className='border-2 border-sky-900 bg-sky-100 pl-2 pr-2 font-extrabold'>{item.product.price * item.quantity} đ</td>
          </tr>
        );
      });
    }
