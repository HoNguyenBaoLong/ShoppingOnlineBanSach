    ...
    const mycart = this.context.mycart.map((item, index) => {
      return (
        <tr key={item.product._id} className="datatable">
          ...
          <td className='border-2 border-sky-900 bg-sky-100 pl-2 pr-2 font-medium'>{item.product.price} đ</td>
          <td className='border-2 border-sky-900 bg-white pl-2 pr-2 font-medium'>{item.quantity}</td>
          <td className='border-2 border-sky-900 bg-sky-100 pl-2 pr-2 font-medium'>{item.product.price * item.quantity} đ</td>
